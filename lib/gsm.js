...
class Modem_text {
    constructor(port, options) {
        ...
        // Send msg queue
        this.sendMsgQueue = [];
        this.FLAG_SEND_MSG = false;

        // Check sendMsgQueue for new msgs
        setInterval(() => {
            if (this.FLAG_SEND_MSG == false & this.sendMsgQueue.length > 0) {
                DEBUG && console.log("[GSM-ARDUINO]: sendMsgQueue ready"); 
                this.FLAG_SEND_MSG = true;
                this.actualSendMsg();
            }
        }, 5000);
        ...
        this.parser.on('data', (buffer) => {
            ...
            if (this.FLAG_NEW_MSG == true) {
                ...
                // hangup the call
                this.serial.write(CMD_HANGUP_CALL);
            }
        });
    }
    ...
    sendMsg(num, text) {
        DEBUG && console.log("[GSM-ARDUINO]: Putting msg in sendMsgQueue");
        this.sendMsgQueue.push({ num: num, text: text });
    }
    actualSendMsg() {
        DEBUG && console.log("[GSM-ARDUINO]: In sending msg");
        let cmd = CMD_SEND_MSG + ":" + this.sendMsgQueue[0].num + ":" + this.sendMsgQueue[0].text + "\r\n";
        this.sendMsgQueue.shift();
        DEBUG && console.log("[GSM-ARDUINO]:" + cmd);
    ...
    checkSignal() {
        ...
        this.serial.write(CMD_CHECK_SIGNAL);
    }
}
